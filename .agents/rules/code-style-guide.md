...
Any violation of this rule is considered a critical issue and must be fixed immediately.
...
Commands must publish Domain Events instead of performing side-effects directly.
...
(Outbox Pattern, Redis Pub/Sub, and Background Worker belong in this layer)
5. API Layer
...
All wallet/money changes must publish MoneyChangedEvent instead of calling other services directly.
7. AI Calls
Atomically reserve quota before calling the provider.
Implement controlled retry + timeout + auto-refund exactly as specified.
After calling AI, the relevant event must be published (no direct calls to notification or quota services).
...
Create file src/lib/utils.ts with the following content and always import from there:TypeScriptimport { type ClassValue, clsx } from "clsx";
...
Rule 0 (Mandatory Event-Driven Architecture) is absolute and non-negotiable.
Always prioritize clean, testable, maintainable, and event-driven code.