...
public class AdminRbacIntegrationTests : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    public async Task AdminRoute_ShouldReject_WhenUserIsNotAdmin()
    {
        ...
        client.DefaultRequestHeaders.Add("X-Test-Role", "User"); // Setup as regular user

        var response = await client.GetAsync("/api/v1/admin/users");

        Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode); // Or Unauthorized based on Asp.Net core behavior
    }
    ...
    public async Task AdminRoute_ShouldSucceed_WhenUserIsAdmin()
    {
        ...
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(TestAuthHandler.AuthenticationScheme);
        client.DefaultRequestHeaders.Add("X-Test-Role", "admin"); // Setup as Admin
