...
public partial class MongoDbContext
{
    private void EnsureReaderCollectionIndexes()
    {
        ReaderRequests.Indexes.CreateOne(new CreateIndexModel<ReaderRequestDocument>(
            Builders<ReaderRequestDocument>.IndexKeys.Ascending(r => r.UserId).Descending(r => r.CreatedAt),
            new CreateIndexOptions { Name = "idx_userid_createdat_desc" }));

        ReaderRequests.Indexes.CreateOne(new CreateIndexModel<ReaderRequestDocument>(
            Builders<ReaderRequestDocument>.IndexKeys.Ascending(r => r.Status).Descending(r => r.CreatedAt),
        ...
        ReaderProfiles.Indexes.CreateOne(new CreateIndexModel<ReaderProfileDocument>(
            Builders<ReaderProfileDocument>.IndexKeys.Ascending(r => r.IsDeleted).Ascending(r => r.Status).Descending(r => r.UpdatedAt),
