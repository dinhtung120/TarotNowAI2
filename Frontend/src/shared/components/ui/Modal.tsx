...
export default function Modal({ 
  ...
  return createPortal(
    <div className={cn('fixed inset-0 tn-z-9999 flex items-center justify-center p-4 pt-24 pb-28 md:p-8 pointer-events-none')} role="dialog" aria-modal="true" aria-label={title}>
      <div className={cn('absolute inset-0 animate-in fade-in tn-modal-backdrop duration-200 pointer-events-auto')} onClick={onClose} />
      <div className={cn(
        'relative z-10 w-full flex flex-col rounded-3xl tn-modal-shell animate-in fade-in zoom-in-95 duration-300 pointer-events-auto', 
        sizeStyles[size],
