...
export default function Modal({ children, description, isOpen, onClose, title, size = 'md', showCloseButton = true }: ModalProps) {
  ...
  return createPortal(
    <div className={cn('fixed inset-0 tn-z-9999 flex items-center justify-center p-4')} role="dialog" aria-modal="true" aria-label={title}>
      <div className={cn('absolute inset-0 animate-in fade-in tn-modal-backdrop duration-200')} onClick={onClose} />
      <div className={cn('relative z-10 w-full rounded-3xl tn-modal-shell animate-in fade-in zoom-in-95 duration-300', sizeStyles[size])}>
