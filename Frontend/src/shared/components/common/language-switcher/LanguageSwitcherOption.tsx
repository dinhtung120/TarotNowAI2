...
export default function LanguageSwitcherOption({ id, flag, isActive, label, onSelect }: LanguageSwitcherOptionProps) {
  return (
    <button type="button" onClick={() => onSelect(id)} className={cn('min-h-11 w-full rounded-xl px-3 py-2.5 text-left text-xs font-semibold transition-all', isActive ? 'border border-[var(--border-focus)] bg-[var(--purple-100)] text-[var(--text-ink)]' : 'tn-surface tn-border-soft border tn-text-secondary hover:tn-surface-strong hover:tn-text-primary')}>
      <span className={cn('inline-flex w-full items-center justify-between gap-3')}>
        ...
        {isActive ? <Check className={cn('h-3.5 w-3.5 text-[var(--purple-accent)]')} /> : null}
      </span>
    </button>
  );
}
