...
export default function LanguageSwitcherMenu({ currentLocale, getLocaleLabel, menuAria, menuTitle, onSelect }: LanguageSwitcherMenuProps) {
  return (
    <div className={cn('absolute right-0 mt-2 w-[12rem] rounded-2xl p-2 tn-panel shadow-[var(--shadow-elevated)]')}>
      <div className={cn('px-2 pb-1 text-[10px] font-black uppercase tracking-[0.18em] tn-text-secondary')}>{menuTitle}</div>
