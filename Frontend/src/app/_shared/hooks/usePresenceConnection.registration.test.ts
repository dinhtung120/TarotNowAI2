...
import { registerPresenceConnectionHandlers } from '@/app/_shared/hooks/usePresenceConnection.registration';
import { invalidateUserStateQueries } from '@/app/_shared/gateways/invalidateUserStateQueries';
import { performClientLogoutCleanup } from '@/features/auth/shared/gateways/clientLogoutCleanup';
...
vi.mock('@/app/_shared/gateways/invalidateUserStateQueries', () => ({
 invalidateUserStateQueries: vi.fn().mockResolvedValue(undefined),
}));

vi.mock('@/features/auth/shared/gateways/clientLogoutCleanup', () => ({
 performClientLogoutCleanup: vi.fn(),
}));

vi.mock('@/app/_shared/gateways/gachaRealtimeDedup', () => ({
 shouldSkipRealtimeGachaInvalidation: vi.fn(() => false),
}));

vi.mock('@/app/_shared/gateways/inventoryRealtimeDedup', () => ({
