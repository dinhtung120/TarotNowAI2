  ...
  return (
    <CallProvider>
      <div className={cn("flex", "h-full", "w-full", "overflow-hidden", "bg-black/40", "backdrop-blur-sm")}>
        {}
        <aside className={cn(isChatRoom ? "hidden" : "flex", "w-full", "max-w-md", "shrink-0", "flex-col", "border-r", "border-white/5")}>
          <ConversationSidebar />
        </aside>

        {}
        <main className={cn(isChatRoom ? "flex" : "hidden", "relative", "min-w-0", "flex-1", "flex-col")}>
          {children}
        </main>
      </div>
