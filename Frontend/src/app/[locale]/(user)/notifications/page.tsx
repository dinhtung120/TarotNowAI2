...
export default function NotificationsPage() {
  ...
  /* filterUnread: false = tất cả, true = chỉ chưa đọc */
  const [filterUnread, setFilterUnread] = useState(false);

  const pageSize = 20;

  /*
   * fetchData: gọi server action lấy danh sách thông báo.
   * useCallback để memo hóa, tránh re-create function mỗi render.
   * Dependency: page, filterUnread → khi thay đổi filter/trang sẽ re-fetch.
   */
  const fetchData = useCallback(async () => {
    ...
    const result = await getNotifications(
      page,
      pageSize,
      filterUnread ? false : undefined /* false = chưa đọc, undefined = tất cả */
    );
    setData(result);
    setLoading(false);
  }, [page, filterUnread]);
  ...
  const handleMarkRead = async (id: string) => {
    ...
    if (result.success) {
      ...
      /* Cập nhật optimistic: đổi isRead=true trong state hiện tại */
      setData((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
  ...
  /* Tính tổng trang cho pagination */
