      ...
      <div className="w-full flex flex-wrap justify-center gap-8">
        {isLoading ? (
          ...
          <div className="text-center text-red-400 p-12 border border-dashed border-red-900/30 bg-red-950/20 rounded-2xl w-full max-w-2xl">
            ...
            <Button variant="secondary" size="sm" onClick={() => window.location.reload()}>Thử lại</Button>
          </div>
        ) : banners?.length ? (
          banners.map((banner) => (
            <GachaBannerCard 
              ...
              banner={banner} 
              onSpin={handleSpin}
              isSpinning={spinMutation.isPending && spinMutation.variables?.bannerCode === banner.code}
              currentPity={optimisticPity[banner.code] ?? banner.userCurrentPity ?? 0}
              hardPityCount={90} // This is fixed by rule
            />
          ))
        ) : (
