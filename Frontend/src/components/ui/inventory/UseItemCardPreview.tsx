...
export default function UseItemCardPreview({ card, label }: UseItemCardPreviewProps) {
  ...
  return (
    <section className={cn('relative overflow-hidden rounded-[1.5rem] border tn-border-soft bg-[#0a0a0c]/40 p-4 transition-all duration-500 hover:tn-border-accent/30')}>
      <div className="absolute -right-4 -top-4 h-16 w-16 rounded-full bg-violet-500/5 blur-2xl" />
        ...
        <div className={cn(
          ...
          {imageUrl ? (
            ...
            <div className={cn('flex h-full w-full items-center justify-center bg-white/[0.02] text-xl tn-text-muted')}>
              <span className="opacity-20 flex flex-col items-center gap-1">
                <span className="text-[10px] font-black uppercase tracking-tighter">No</span>
          ...
          {card.stats ? (
            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-widest tn-text-muted opacity-40">Level</span>
                <span className="text-xs font-black tn-text-secondary">Lv {card.stats.level}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-widest tn-text-muted opacity-40">Attack</span>
                <span className="text-xs font-black text-rose-400">{formatCardStat(card.stats.totalAtk)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-widest tn-text-muted opacity-40">Defense</span>
                <span className="text-xs font-black text-sky-400">{formatCardStat(card.stats.totalDef)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-widest tn-text-muted opacity-40">Experience</span>
                <span className="text-[10px] font-bold tn-text-secondary">
                  {formatCardStat(card.stats.currentExp)} <span className="opacity-30">/</span> {formatCardStat(card.stats.expToNextLevel)}
                </span>
              </div>
            </div>
          ) : (
            <p className="text-[10px] font-medium tn-text-secondary opacity-40 italic">
              No stats available
            </p>
          )}
        </div>
