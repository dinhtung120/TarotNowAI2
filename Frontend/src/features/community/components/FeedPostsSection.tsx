...
export function FeedPostsSection({ activeVisibility, data, hasNextPage, isFetchingNextPage, labels, onReport, onLoadMore }: FeedPostsSectionProps) {
 ...
 return (
  <div className="space-y-4">
   ...
   {hasNextPage ? <button onClick={onLoadMore} disabled={isFetchingNextPage} className="w-full py-3 mt-4 text-sm text-[#8a2be2] border border-[#2a2b3d] rounded-xl hover:bg-[#2a2b3d]/50 transition-colors">{isFetchingNextPage ? labels.loadingMore : labels.loadMore}</button> : null}
