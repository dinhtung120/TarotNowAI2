...
const asNumber = (value: unknown, fallback = 0) => (typeof value === 'number' && Number.isFinite(value)) ? value : (typeof value === 'string' && Number.isFinite(Number(value)) ? Number(value) : fallback);
...
export async function listReaders(page = 1, pageSize = 12, specialty = '', status = '', searchTerm = ''): Promise<ActionResult<ListReadersResponse>> {
 try {
  const params = new URLSearchParams({ page: page.toString(), pageSize: pageSize.toString() });
  if (specialty) params.append('specialty', specialty);
  if (status) params.append('status', status);
  if (searchTerm) params.append('searchTerm', searchTerm);
  const result = await serverHttpRequest<ReaderApi>(`/readers?${params.toString()}`, { method: 'GET', cache: 'no-store', fallbackErrorMessage: FAIL_LIST_READERS });
  if (!result.ok) {
   logger.error('[ReaderAction] listReaders', result.error, { status: result.status, page, pageSize, specialty, statusFilter: status, searchTerm });
...
export async function listFeaturedReaders(limit = 4): Promise<ActionResult<ReaderProfile[]>> {
 try {
  const result = await serverHttpRequest<{ readers?: ReaderProfile[]; Readers?: ReaderProfile[] }>(`/readers?page=1&pageSize=${limit}`, { method: 'GET', next: { revalidate: 120 }, fallbackErrorMessage: FAIL_LIST_FEATURED });
  if (!result.ok) {
...
export async function getReaderProfile(userId: string): Promise<ActionResult<ReaderProfile>> {
 try {
  const result = await serverHttpRequest<ReaderProfile>(`/reader/profile/${userId}`, { method: 'GET', cache: 'no-store', fallbackErrorMessage: FAIL_GET_READER });
  if (!result.ok) {
