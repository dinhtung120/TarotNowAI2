  ...
  return (
    <div className={cn('mx-auto max-w-2xl animate-in fade-in slide-in-from-bottom-8 space-y-10 px-4 py-20 duration-1000 sm:px-6')}>
      ...
      <form onSubmit={handleSubmit} className={cn('space-y-8')}>
        ...
        <ReaderApplyIntroField introText={introText} setIntroText={setIntroText} />
        {message ? <ReaderApplyFeedbackMessage message={message} type={messageType} /> : null}
        <ReaderApplySubmitButton submitting={submitting} disabled={submitting || introText.length < 20} />
      </form>
    </div>
  );
}
