 ...
 return (
  <div className={cn("h-[60vh] flex flex-col items-center justify-center space-y-6")}>
   <div className={cn("w-24 h-24 tn-panel-soft rounded-full flex items-center justify-center mb-2 shadow-inner")}>
    <User className={cn("w-10 h-10 text-[var(--text-tertiary)] opacity-50")} />
   </div>
   <p className={cn("text-[var(--text-secondary)] text-sm font-medium")}>{description}</p>
   <Button variant="ghost" onClick={onBack} className={cn("text-[var(--text-secondary)] hover:tn-text-primary mt-4")}>
