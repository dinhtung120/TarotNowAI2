 ...
 return (
  <GlassCard
   className={cn(
    ...
    item.isRead ? 'opacity-70' : 'border-l-2 tn-border-accent',
   )}
  >
   <div className={cn('flex items-start gap-4')}>
    <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center shrink-0 tn-bg-glass')}>
    ...
    <div className={cn('flex-1 min-w-0')}>
     <div className={cn('flex items-start justify-between gap-2')}>
      ...
      <span className={cn('tn-text-10 font-medium tn-text-muted whitespace-nowrap shrink-0')}>{timeLabel}</span>
     </div>
     ...
     {!item.isRead ? (
      <button
       ...
       className={cn('mt-2 flex items-center gap-1.5 tn-text-10 font-bold uppercase tracking-widest tn-text-accent-soft tn-hover-text-accent transition-colors')}
      >
