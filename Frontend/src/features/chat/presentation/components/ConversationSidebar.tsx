...
export default function ConversationSidebar() {
 ...
 return (
  <div className={cn('flex flex-col h-full overflow-hidden gap-3 p-3 md:p-4')}>
   ...
   <div className={cn('flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar')}>
    {loading ? (
     <div className={cn('h-32 flex items-center justify-center')}>
      <Loader2 className={cn('w-5 h-5 animate-spin text-[var(--text-secondary)]')} />
     </div>
    ) : null}
    {!loading && conversations.length === 0 ? (
     <div className={cn('h-40 flex flex-col items-center justify-center gap-2 text-[var(--text-secondary)] text-center px-4')}>
