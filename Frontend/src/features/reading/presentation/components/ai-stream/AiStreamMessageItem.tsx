 ...
 return (
  <div className={cn("flex w-full", isUser ? "justify-end px-3" : "justify-start")}>
   <div className={cn("flex", isUser ? "tn-maxw-95 flex-row-reverse" : "w-full flex-row")}>
    <div className={cn("py-5", isUser ? "px-6 tn-bg-warning-10 border tn-border-warning-20 rounded-3xl rounded-tr-none" : "w-full px-6 tn-bg-accent-10 border tn-border-accent-20 rounded-3xl rounded-tl-none tn-ai-stream-markdown")}>
     {isUser ? <p className={cn("tn-text-warning")}>{message.content}</p> : <ReactMarkdown>{message.content}</ReactMarkdown>}
