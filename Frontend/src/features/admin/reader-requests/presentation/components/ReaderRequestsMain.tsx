...
export function ReaderRequestsMain(props: ReaderRequestsMainProps) {
 ...
 return (
  <div className={cn('max-w-5xl mx-auto tn-page-x py-16 space-y-8 animate-in fade-in duration-700')}>
   ...
   <ReaderRequestsContent getAdminNote={props.getAdminNote} emptyLabel={props.t('reader_requests.states.empty')} labels={labels} loading={props.loading} locale={props.locale} onAdminNoteChange={props.updateAdminNote} onApprove={(requestId) => props.handleProcess(requestId, 'approve')} onReject={(requestId) => props.handleProcess(requestId, 'reject')} onSelectRequest={props.selectRequest} page={props.page} paginationLabel={props.t('reader_requests.pagination.summary', { page: props.page, total: props.totalPages })} processingId={props.processing} requests={props.requests} selectedRequestId={props.selectedRequestId} setPage={props.setPage} totalPages={props.totalPages} />
  </div>
 );
}
