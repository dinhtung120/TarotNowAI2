...
export function AdminReadingsFilters({ username, spreadType, startDate, endDate, onUsernameChange, onSpreadTypeChange, onStartDateChange, onEndDateChange, onSubmit }: AdminReadingsFiltersProps) {
 ...
 return (
  <form onSubmit={submitWithValidation} className={cn("p-6 tn-rounded-2_5xl tn-bg-accent-5 border tn-border-soft shadow-inner flex flex-wrap items-end gap-6")}>
   ...
   <Button type="submit" variant="primary" className={cn("px-8 py-4 shrink-0 shadow-md flex items-center justify-center tn-minw-140")}><Search className={cn("w-4 h-4")} />{t("readings.filters.submit")}</Button>
  </form>
 );
}
