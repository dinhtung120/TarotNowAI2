 ...
 return (
  <td className={cn("px-8 py-5")}>
   <div className={cn("flex items-center gap-3")}>
    ...
    <div>
     <div className={cn("tn-text-11 font-black tn-text-primary uppercase tracking-tighter drop-shadow-sm")}>{username || systemUserLabel}</div>
     <div className={cn("tn-text-9 font-bold tn-text-tertiary italic tracking-tighter")}>{userIdPrefixLabel(userId.split("-")[0])}</div>
    </div>
   </div>
  </td>
 );
