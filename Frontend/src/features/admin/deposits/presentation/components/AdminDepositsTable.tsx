...
export function AdminDepositsTable({ locale, orders, loading, page, totalCount, processingId, labels, onApprove, onReject, onPrev, onNext }: AdminDepositsTableProps) {
 return (
  <GlassCard className={cn("!p-0 !tn-rounded-2_5xl overflow-hidden text-left")}>
   <div className={cn("overflow-x-auto custom-scrollbar")}>
    <table className={cn("w-full text-left")}>
     ...
     <AdminDepositsTableBody locale={locale} orders={orders} loading={loading} processingId={processingId} labels={labels} onApprove={onApprove} onReject={onReject} />
    </table>
   </div>
