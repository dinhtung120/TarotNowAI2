...
export function LeaderboardCurrencyTabs({ currency, goldLabel, diamondLabel, onChange }: LeaderboardCurrencyTabsProps) {
 ...
 return (
  <div className="flex bg-slate-900/60 p-1.5 rounded-2xl border border-slate-700/50 backdrop-blur-md">
   <button onClick={() => onChange("gold")} className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${tabClassName("gold")}`}><Coins className="w-4 h-4" />{goldLabel}</button>
   <button onClick={() => onChange("diamond")} className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${tabClassName("diamond")}`}><Diamond className="w-4 h-4" />{diamondLabel}</button>
  </div>
 );
}
