 ...
 return (
  <div className={cn('p-6 relative z-10')}>
   <div className={cn('flex justify-between items-start')}>
    ...
    <button type="button" onClick={onOpenOdds} className={cn('p-1.5 rounded-lg bg-stone-800/50 text-stone-400 tn-hover-stone-200 transition-colors border border-stone-700/50')} title={viewOddsLabel}>
     <HelpCircle className={cn('w-4 h-4')} />
    </button>
   </div>
   <p className={cn('tn-text-11 font-medium text-stone-400 mt-2 uppercase tracking-tight leading-relaxed line-clamp-2')}>{description}</p>
  </div>
 );
}
