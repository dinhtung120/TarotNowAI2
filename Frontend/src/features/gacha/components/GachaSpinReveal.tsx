...
export function GachaSpinReveal({ result, isOpen, onClose }: GachaSpinRevealProps) {
 ...
 return (
  <Modal isOpen={isOpen} onClose={() => vm.phase === "revealed" && onClose()} title={vm.t("spinResult")} size="lg" showCloseButton={vm.phase === "revealed"}>
   <div className={cn("flex", "min-h-96", "flex-col", "items-center", "justify-center", result.items.length > 1 ? "w-full" : "mx-auto max-w-2xl")}>
    {vm.phase === "firing" ? (
    ...
    {vm.phase === "revealed" ? (
     <div className={cn("flex", "h-full", "w-full", "flex-col", "items-center")}>
      ...
      <div className={cn("w-full", result.items.length > 1 ? "grid grid-cols-5 gap-4" : "flex justify-center")}>
      ...
      <Button variant="brand" size="lg" className={cn("mt-10", "min-w-xs")} onClick={onClose}>
       {vm.t("awesome")}
      </Button>
     </div>
    ) : null}
