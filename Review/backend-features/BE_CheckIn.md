...
- Mục tiêu nghiệp vụ: review backend feature `CheckIn` thuộc nhóm `Engagement`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/CheckIn` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `CheckIn`.
...
- API controllers hoặc application services gọi command/query của `CheckIn`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `CheckIn`.
