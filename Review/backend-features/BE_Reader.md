...
- Mục tiêu nghiệp vụ: review backend feature `Reader` thuộc nhóm `Reader/Reading domain`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Reader` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Reader`.
...
- API controllers hoặc application services gọi command/query của `Reader`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Reader`.
