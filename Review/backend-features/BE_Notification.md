...
- Mục tiêu nghiệp vụ: review backend feature `Notification` thuộc nhóm `Community & Side Effects`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Notification` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Notification`.
...
- API controllers hoặc application services gọi command/query của `Notification`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Notification`.
