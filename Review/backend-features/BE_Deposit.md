...
- Mục tiêu nghiệp vụ: review backend feature `Deposit` thuộc nhóm `Finance`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Deposit` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Deposit`.
...
- API controllers hoặc application services gọi command/query của `Deposit`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Deposit`.
