...
- Mục tiêu nghiệp vụ: review backend feature `Admin` thuộc nhóm `Identity & Access / Compliance Admin`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Admin` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Admin`.
...
- API controllers hoặc application services gọi command/query của `Admin`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Admin`.
