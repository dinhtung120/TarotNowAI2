...
- Mục tiêu nghiệp vụ: review backend feature `Gamification` thuộc nhóm `Engagement`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Gamification` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Gamification`.
...
- API controllers hoặc application services gọi command/query của `Gamification`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Gamification`.
