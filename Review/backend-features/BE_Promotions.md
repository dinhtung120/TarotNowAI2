...
- Mục tiêu nghiệp vụ: review backend feature `Promotions` thuộc nhóm `Growth`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Promotions` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Promotions`.
...
- API controllers hoặc application services gọi command/query của `Promotions`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Promotions`.
