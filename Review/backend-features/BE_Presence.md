...
- Mục tiêu nghiệp vụ: review backend feature `Presence` thuộc nhóm `Conversation & Realtime`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Presence` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Presence`.
...
- API controllers hoặc application services gọi command/query của `Presence`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Presence`.
