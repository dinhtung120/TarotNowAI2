...
- Mục tiêu nghiệp vụ: review backend feature `Chat` thuộc nhóm `Conversation & Realtime`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Chat` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Chat`.
...
- API controllers hoặc application services gọi command/query của `Chat`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Chat`.
