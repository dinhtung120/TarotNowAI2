...
- Mục tiêu nghiệp vụ: review backend feature `Profile` thuộc nhóm `User Profile`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Profile` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Profile`.
...
- API controllers hoặc application services gọi command/query của `Profile`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Profile`.
