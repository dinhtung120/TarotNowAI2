...
- Mục tiêu nghiệp vụ: review backend feature `UserContext` thuộc nhóm `Identity & Access`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/UserContext` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `UserContext`.
...
- API controllers hoặc application services gọi command/query của `UserContext`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `UserContext`.
