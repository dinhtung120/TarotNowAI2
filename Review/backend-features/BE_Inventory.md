...
- Mục tiêu nghiệp vụ: review backend feature `Inventory` thuộc nhóm `Engagement`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Inventory` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Inventory`.
...
- API controllers hoặc application services gọi command/query của `Inventory`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Inventory`.
