...
- Mục tiêu nghiệp vụ: review backend feature `Home` thuộc nhóm `Reader/Reading domain`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Home` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Home`.
...
- API controllers hoặc application services gọi command/query của `Home`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Home`.
