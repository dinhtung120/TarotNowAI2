...
- Mục tiêu nghiệp vụ: review backend feature `Legal` thuộc nhóm `Compliance`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Legal` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Legal`.
...
- API controllers hoặc application services gọi command/query của `Legal`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Legal`.
