...
- Mục tiêu nghiệp vụ: review backend feature `Gacha` thuộc nhóm `Engagement`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Gacha` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Gacha`.
...
- API controllers hoặc application services gọi command/query của `Gacha`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Gacha`.
