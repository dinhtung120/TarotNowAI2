...
- Mục tiêu nghiệp vụ: review backend feature `Escrow` thuộc nhóm `Conversation & Finance`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Escrow` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Escrow`.
- Requested Domain Event: xác định các `*RequestedDomainEvent` và `*RequestedDomainEventHandler` nếu có.
- Realtime event: cần rà soát SignalR/Redis bridge và event name liên quan.
...
- API controllers hoặc application services gọi command/query của `Escrow`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Escrow`.
- Transaction boundary: review `CommandTransactionBehavior` và transaction coordinator nếu state mutation quan trọng.
- Idempotency key path: Bắt buộc rà soát idempotency key path vì module có finance/quota/AI/reward concern.
- Outbox/realtime bridge path: Bắt buộc rà outbox/realtime bridge path.
