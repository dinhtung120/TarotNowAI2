...
- Feature source: `Backend/src/TarotNow.Application/Features/Withdrawal`.
- API/controller source cần đối chiếu: `Backend/src/TarotNow.Api` với grep `Withdrawal`.
- Infrastructure source cần đối chiếu: `Backend/src/TarotNow.Infrastructure` với repositories/services liên quan `Withdrawal`.
- Test/guard source: `Backend/tests/TarotNow.ArchitectureTests/*.cs` và `Backend/tests` grep `Withdrawal`.

## 2. Entry points & luồng chính

- Commands/Queries: source nằm dưới `Features/Withdrawal/Commands` và/hoặc `Features/Withdrawal/Queries` nếu thư mục tồn tại.
...
- API controllers hoặc background/event handlers gọi command/query thuộc `Withdrawal`.
- Frontend feature tương ứng nếu có route/API contract liên quan.
- Cross-feature events nếu `Withdrawal` nhận hoặc phát domain events.
...
Evidence dữ liệu cụ thể: PostgreSQL withdrawal_requests/reader_payout_profiles/wallet_transactions; admin approval and payout boundary critical.
...
- Feature tests: tìm bằng `find Backend/tests -type f | grep -E 'Withdrawal|Architecture|EventDriven'`.
...
- Mức độ phù hợp kiến trúc: cần audit chi tiết theo source files trong `Features/Withdrawal`; khung review này đã neo đúng source và guard.
- Evidence quan trọng: `Features/Withdrawal`, architecture tests, Infrastructure persistence/repositories, API controllers.
