...
- Mục tiêu nghiệp vụ: review backend feature `Withdrawal` thuộc nhóm `Finance`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Withdrawal` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Withdrawal`.
...
- API controllers hoặc application services gọi command/query của `Withdrawal`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Withdrawal`.
