...
- Mục tiêu nghiệp vụ: review backend feature `Community` thuộc nhóm `Community & Moderation`.
- Module liên quan: `Backend/src/TarotNow.Application/Features/Community` và các Domain/Infrastructure/API tương ứng.
...
- API/Command/Query/Route chính: rà soát controller/API và handlers trỏ tới feature `Community`.
...
- API controllers hoặc application services gọi command/query của `Community`.
...
- Entity/Document chính: xác định từ Domain entities, EF configs, Mongo documents liên quan `Community`.
