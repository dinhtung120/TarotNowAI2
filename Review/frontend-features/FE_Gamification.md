...
- Mục tiêu nghiệp vụ: review frontend feature `Gamification`.
- Module liên quan: `Frontend/src/features/gamification` nếu tồn tại, route trong `Frontend/src/app/[locale]`, shared/i18n liên quan.
...
- API/Command/Query/Route chính: xác định page/layout/API route gọi feature `Gamification`.
...
- App Router page/layout hoặc shared prefetch runner import feature `Gamification`.
...
- public.ts exports: xác định `Frontend/src/features/*/public.ts` liên quan `Gamification`.
