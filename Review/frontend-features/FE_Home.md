...
- Feature source: `Frontend/src/features/home`.
- Public export: `Frontend/src/features/home/public.ts` nếu tồn tại.
- App routes cần đối chiếu: `Frontend/src/app/[locale]` grep `home` hoặc route nghiệp vụ tương ứng.
...
- Route/page/layout: xác định bằng `find Frontend/src/app -type f | grep -E 'home|home'`.
- Feature public surface: `Frontend/src/features/home/public.ts` là boundary ưu tiên cho app imports.
- Components/hooks/actions: nằm trong `Frontend/src/features/home` theo cấu trúc hiện tại.
...
- App Router page/layout/API route import feature `home`.
...
- Local UI state: rà component/hook trong `Frontend/src/features/home`.
...
- Feature tests: tìm trong `Frontend/tests` hoặc colocated tests với grep `home`.
...
- Mức độ phù hợp kiến trúc: file đã neo đúng source feature `home` và guard frontend; cần audit từng route/action để kết luận pass cuối cùng.
- Evidence quan trọng: `Frontend/src/features/home`, `Frontend/src/app/[locale]`, `Frontend/src/shared/server/prefetch`, `Frontend/messages`, `Frontend/scripts`.
