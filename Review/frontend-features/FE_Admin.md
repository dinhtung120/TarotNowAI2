...
- Mục tiêu nghiệp vụ: review frontend feature `Admin`.
- Module liên quan: `Frontend/src/features/admin` nếu tồn tại, route trong `Frontend/src/app/[locale]`, shared/i18n liên quan.
...
- API/Command/Query/Route chính: xác định page/layout/API route gọi feature `Admin`.
...
- App Router page/layout hoặc shared prefetch runner import feature `Admin`.
...
- public.ts exports: xác định `Frontend/src/features/*/public.ts` liên quan `Admin`.
