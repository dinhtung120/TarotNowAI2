...
- Mục tiêu nghiệp vụ: review frontend feature `Notifications`.
- Module liên quan: `Frontend/src/features/notifications` nếu tồn tại, route trong `Frontend/src/app/[locale]`, shared/i18n liên quan.
...
- API/Command/Query/Route chính: xác định page/layout/API route gọi feature `Notifications`.
...
- App Router page/layout hoặc shared prefetch runner import feature `Notifications`.
...
- public.ts exports: xác định `Frontend/src/features/*/public.ts` liên quan `Notifications`.
