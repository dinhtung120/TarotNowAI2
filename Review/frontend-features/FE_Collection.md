...
- Feature source: `Frontend/src/features/collection`.
- Public export: `Frontend/src/features/collection/public.ts` nếu tồn tại.
- App routes cần đối chiếu: `Frontend/src/app/[locale]` grep `collection` hoặc route nghiệp vụ tương ứng.
...
- Route/page/layout: xác định bằng `find Frontend/src/app -type f | grep -E 'collection|collection'`.
- Feature public surface: `Frontend/src/features/collection/public.ts` là boundary ưu tiên cho app imports.
- Components/hooks/actions: nằm trong `Frontend/src/features/collection` theo cấu trúc hiện tại.
...
- App Router page/layout/API route import feature `collection`.
...
- Local UI state: rà component/hook trong `Frontend/src/features/collection`.
...
- Feature tests: tìm trong `Frontend/tests` hoặc colocated tests với grep `collection`.
...
- Mức độ phù hợp kiến trúc: file đã neo đúng source feature `collection` và guard frontend; cần audit từng route/action để kết luận pass cuối cùng.
- Evidence quan trọng: `Frontend/src/features/collection`, `Frontend/src/app/[locale]`, `Frontend/src/shared/server/prefetch`, `Frontend/messages`, `Frontend/scripts`.
