...
- Bắt buộc trên server (không commit): `deploy/.env.prod` trên DB/BE/FE host.
...
cd /opt/tarotnow
git clone <REPO_URL> TarotNowAI2
cd TarotNowAI2
...
docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml ps postgres mongodb redis
docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml logs --tail=100 postgres mongodb redis
```
...
cd /opt/tarotnow/TarotNowAI2
docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml ps backend
...
cd /opt/tarotnow/TarotNowAI2
docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml ps frontend reverse-proxy
...
3. Trên BE host:
   ```bash
   curl -fsS http://localhost:5037/api/v1/health/ready
   docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml logs --tail=200 backend
   ```
...
# BE host
ssh -i <KEY_PATH> <USER>@<BE_HOST> "cd /opt/tarotnow/TarotNowAI2 && docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml ps backend"

# DB host
ssh -i <KEY_PATH> <USER>@<DB_HOST> "cd /opt/tarotnow/TarotNowAI2 && docker compose --env-file deploy/.env.prod -f docker-compose.prod.yml ps postgres mongodb redis"
```
